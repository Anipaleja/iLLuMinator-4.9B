...
class AppleSiliconOptimizer:
    ...
    def get_optimal_batch_size(model_params: int, available_memory_gb: float):
        """Calculate optimal batch size for Apple Silicon"""
        # Conservative estimate for 4.9B model on 16GB RAM
        if model_params > 4e9:  # 4B+ parameters
            if available_memory_gb >= 14:
                return 1  # Single sample for maximum stability
            else:
                return 1  # Ultra conservative
        return 2
...
def main():
    ...
    model = iLLuMinator4_7B(
        ...
        n_layers=30,       # Keep full layer count
        n_heads=28,        # Keep full attention heads
        d_ff=14336,        # Keep full feedforward dimension
